...
from nonebug.typing import Union, Optional
...
class Constructor:
    ...
    def set_message(self, message: Union[str, MessageSegment, Message], user_id: int,
        ...
        kwargs['raw_message'] = raw_message if raw_message else str(
            message)  # check_to_me较为困难，所以不检查，由用户设定原始信息
    ...
    def add_api(self, action: str, data: Optional[dict], result: dict, mock: bool = False) -> None:
        if isinstance(self.bot, CQHTTPBot):
            api_type = get_cqhttp_api(action)
        else:
            raise NotImplementedError(
                "add_api is not supported in non-cqhttp adapter, please use add_api_model")
        if not mock and not data:
            raise ValueError("data cannot be empty if mock is false")
        self.api_list.append(
            api_type(action=action, data=data, result=result, mock=mock))

    def add_mock_api(self, action: str, result: dict) -> None:
    ...
    async def run(self) -> None:
        ...
        await handle_testcase(testcase)
